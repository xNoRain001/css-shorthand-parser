(function (global, factory) {
  ...
  var durationAndDelayParser = function durationAndDelayParser(value, res, prefix) {
    ...
    value = value.replace(/\s?(\d+m?s)/g, function (_, $1) {
      ...
      counter++;
      if (counter === 1) {
        res["".concat(prefix, "-duration")] = $1;
      } else if (counter === 2) {
        res["".concat(prefix, "-delay")] = $1;
      }
      return '';
    });
    return value;
  ...
  var transitionParser = function transitionParser(value) {
    var res = {
    ...
    var flagFnsMap = {
      'transition-timing-function': function transitionTimingFunction(v) {
        return animationTimingFunctionKeywordValues.hasOwnProperty(v) || /(.*?\(.*?\))/.test(v);
      },
      'transition-property': function transitionProperty(v) {
        return v;
      }
    };
